        ...
        <section id="system" className="bg-black text-white py-12 md:py-20 relative z-20">
            ...
            <div className="max-w-[90rem] mx-auto px-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-3 auto-rows-[minmax(160px,auto)]">
                    ...
                    {/* Item 1: Large - Targeting */}
                    <ServiceCard
                        className="md:col-span-2 md:row-span-1"
                        number="01 //"
                        title="SMART MATCHING"
                        desc="We find investors who actually want what you're building. No random guessing."
                        gradient="radial-gradient(circle at top right, rgba(100,50,255,0.4), transparent 70%)"
                    />

                    {/* Item 2: Small - Deliverability */}
                    <ServiceCard
                        className="md:col-span-1 md:row-span-1"
                        number="02 //"
                        title="NO SPAM FOLDERS"
                        desc="We set up your email so it always lands in the main inbox. They will see it."
                        gradient="linear-gradient(to bottom left, rgba(255,100,0,0.2), transparent)"
                    />

                    {/* Item 3: Small - Persistence */}
                    <ServiceCard
                        className="md:col-span-1 md:row-span-1"
                        number="03 //"
                        title="WE FOLLOW UP"
                        desc="Investors get distracted. We keep emailing them for you until they reply."
                        gradient="linear-gradient(to top right, rgba(0,255,100,0.2), transparent)"
                    />

                    {/* Item 4: Large - Copywriting */}
                    <ServiceCard
                        className="md:col-span-2 md:row-span-1"
                        number="04 //"
                        title="BETTER STORY"
                        desc="We rewrite your pitch. We make it simple so they get it immediately."
                        gradient="radial-gradient(circle at bottom left, rgba(50,50,50,0.8), transparent 60%)"
                    />

                    {/* Item 5: Full Width - Data/Access */}
                    <ServiceCard
                        className="md:col-span-3 md:row-span-1 min-h-[160px]"
                        number="05 //"
                        title="HIDDEN MONEY"
                        desc="We know who is investing right now, even if they haven't announced it yet."
