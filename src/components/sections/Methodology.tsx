        ...
        <section id="system" className="bg-black text-white py-12 md:py-20 relative z-20">
            ...
            <div className="max-w-[90rem] mx-auto px-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-3 auto-rows-[minmax(160px,auto)]">
                    ...
                    {/* Item 1: Database Access (Large) */}
                    <ServiceCard
                        className="md:col-span-2 md:row-span-1"
                        number="01 //"
                        title="DATABASE ACCESS"
                        desc="50,000+ VCs. Thesis-matched. Check size filtered. Updated daily. No fluff."
                        gradient="radial-gradient(circle at top right, rgba(100,50,255,0.4), transparent 70%)"
                    />

                    {/* Item 2: Analytics Engine (Small - New) */}
                    <ServiceCard
                        className="md:col-span-1 md:row-span-1"
                        number="02 //"
                        title="ANALYTICS ENGINE"
                        desc="Real-time funnel tracking using proprietary pixel technology."
                        gradient="linear-gradient(to bottom left, rgba(255,100,0,0.2), transparent)"
                    />

                    {/* Item 3: Asset Pre-Screening (Small - Tallish or Standard - New) */}
                    <ServiceCard
                        className="md:col-span-1 md:row-span-1"
                        number="03 //"
                        title="ASSET PRE-SCREENING"
                        desc="AI-driven due diligence that flags risks before you pitch."
                        gradient="linear-gradient(to top right, rgba(0,255,100,0.2), transparent)"
                    />

                    {/* Item 4: Outreach Infrastructure (Large) */}
                    <ServiceCard
                        className="md:col-span-2 md:row-span-1"
                        number="04 //"
                        title="OUTREACH INFRASTRUCTURE"
                        desc="Private domains. Warm IPs. Deliverability monitoring. We handle the technical nightmare."
                        gradient="radial-gradient(circle at bottom left, rgba(50,50,50,0.8), transparent 60%)"
                    />

                    {/* Item 5: Narrative (Full Width or Very Large) */}
                    <ServiceCard
                        className="md:col-span-3 md:row-span-1 min-h-[160px]"
                        number="05 //"
                        title="NARRATIVE CALIBRATION"
                        desc="We don't write decks. We optimize your pitch for maximum reply velocity. Our team of ex-VCs reconstructs your narrative to align with current market theses."
